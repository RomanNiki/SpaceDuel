...
namespace Factories.EntitiesFactories.Projectiles
{
    public abstract class ProjectileEntityFactorySo : EntityFactoryFromSo
    {
        ...
        [SerializeField] private DamageRequest _damage = new() { Value = 1f };
        [SerializeField] private Friction _friction = new() { Value = 0.1f };
        [SerializeField] private Mass _mass = new() { Value = 0.1f };

        public override Entity CreateEntity(in World world)
        {
            var entity = world.CreateEntity();
            OnCreateEntity(entity, world);
            ...
            world.AddComponentToEntity(entity, new DamageContainer() { DamageRequest = _damage });
            return entity;
        }

        protected abstract void OnCreateEntity(Entity entity, in World world);
    }
}